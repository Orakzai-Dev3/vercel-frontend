...
function Men_Shearling_Jacket() {
    ...
    useEffect(() => {
        const fetchData = async () => {
          try {
            setLoading(true)
            const response = await fetch(`https://railway-backend-production-08c2.up.railway.app/men/leather-wallets?color=${color}&size=${size}&price${price}&sort=${sort}`, {
              method: "GET",
                headers: {
  ...
  return (
    ...
    <div className=' '>
        ...
        <div onClick={()=>setShow(!show)} className=' h-full w-full'>
            ...
            <div className='flex flex-col gap-1 my-4'>
                <h1 className='tracking-tighter cursor-pointer hover:font-semibold duration-500'>Leather Wallets
                (13)</h1>
    ...
    <div>
        <img className='hidden lg:block' src="https://cdn.leatherscin.com/img/plpbanner/leather-wallets-banner-2024.webp" alt="" />
        <img className='block lg:hidden w-full' src="https://cdn.leatherscin.com/img/plpbanner/leather-wallets-banner-2024-mobile.webp" alt="" />
    </div>
    <div className='max-w-[85vw] m-auto flex gap-8'>
        ...
        <div className=' w-full lg:basis-[80%]'>
            <div className='flex flex-col lg:flex-row justify-between my-2'>
                ...
                <h1 className='text-xl xl:text-2xl py-2 lg:py-1 text-[#552c1a] underline font-semibold tracking-tighter'> LEATHER WALLETS</h1>
